            ...
            if let Some(data) = line.strip_prefix("data: ") {
                ...
                if let Ok(parsed) = serde_json::from_str::<Value>(data) {
                    if let Some(content) = parsed["choices"][0]["delta"]["content"].as_str() {
                        ...
                        payload.as_object_mut().unwrap().insert("status".to_string(), Value::String("streaming".to_string()));
                        payload.as_object_mut().unwrap().insert("delta".to_string(), Value::String(content.to_string()));
    ...
    for _ in 0..max_rounds {
        ...
        if let Some(calls) = tool_calls {
            ...
            for call in calls {
                ...
                let result = crate::tools::execute_tool_async(name, args.clone(), installed_plugins, config, app).await;
